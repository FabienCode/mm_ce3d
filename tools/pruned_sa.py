...
def main():
    ...
    for i, layer in model.named_modules():
        if 'SA_modules.3' in i:
            if isinstance(layer, nn.BatchNorm2d):
                total_sa1 += layer.weight.data.shape[0]

    bn = torch.zeros(total_sa1)
    index = 0
    for i, layer in model.named_modules():
        if 'SA_modules.3' in i:
            if isinstance(layer, nn.BatchNorm2d):
    ...
    for [(i, m0), m1] in zip(model.named_modules(), pruned_model.modules()):
        ...
        # ===========
        elif 'FP_modules.0.mlps.layer0.conv' in i:
        ...
        elif isinstance(m0, nn.Conv1d) or isinstance(m0, nn.BatchNorm1d) or isinstance(m0, nn.Conv2d) or isinstance(m0, nn.BatchNorm2d):
            m1.weight.data = m0.weight.data.clone()
    new_parameters = obtain_num_parameters(pruned_model)
    print("\new model sum of parameters:", new_parameters)
    save_checkpoint(pruned_model, 'pruned_sa3_0.3_votenet.pth')
