...
class cof_BasePointSAModule(nn.Module):
    ...
    def __init__(self,
        ...
        super(cof_BasePointSAModule, self).__init__()
        ...
        # self.xyz_embeddings = nn.Linear(3, self.mlp_channels[0][-1])
        self.cof_attention = BasicLayer(self.mlp_channels[0][-1], 2, 32)
    ...
    def forward(
        ...
        for i in range(len(self.groupers)):
            ...
            # new_xyz_embedding = self.xyz_embeddings(new_xyz)
            new_features = self.cof_attention(new_xyz,
                                              new_features.permute(0, 2, 1).contiguous(),
                                              new_xyz.shape[1]).permute(0, 2, 1).contiguous()
            # mlp_features = []
            # ori_features = []
            # cof_features = []
            # for j in range(len(self.mlps[i])):
            #     ori_layer_feature = self.mlps[i][j](grouped_results)
            #     pool_feature = self._pool_features(ori_layer_feature)
            #     ori_features.append(pool_feature)
            #     tmp_xyz_embedding = self.xyz_embeddings[i][j](new_xyz)
            #     tmp_cof_feature = self.cof_attention[i][j](tmp_xyz_embedding,
            #                                                pool_feature.permute(0, 2, 1).contiguous(),
            #                                                tmp_xyz_embedding.shape[1]).permute(0, 2, 1).contiguous()
            #     grouped_results = tmp_cof_feature.unsqueeze(-1)
            #     cof_features.append(tmp_cof_feature)
...
class cofbase_PointSAModuleMSG(cof_BasePointSAModule):
    ...
    def __init__(self,
                 ...
                 normalize_xyz=False,
                 bias='auto'):
        super(cofbase_PointSAModuleMSG, self).__init__(
            num_point=num_point,
            radii=radii,
            ...
            for i in range(len(mlp_channel) - 1):
                mlp.add_module(
                    ...
                    ConvModule(
                        ...
                        norm_cfg=norm_cfg,
                        bias=bias))
                # xyz_embedding.add_module(f'layer{i}', nn.Linear(3, mlp_channel[i + 1]))
                # cof_attention.add_module(f'layer{i}', BasicLayer(mlp_channel[i + 1], 2, 32))

            self.mlps.append(mlp)
            # self.xyz_embeddings.append(xyz_embedding)
            # self.cof_attention.append(cof_attention)


@SA_MODULES.register_module()
class cofbase_PointSAModule(cofbase_PointSAModuleMSG):
    ...
    def __init__(self,
                 ...
                 fps_sample_range_list=[-1],
                 normalize_xyz=False):
        super(cofbase_PointSAModule, self).__init__(
            mlp_channels=[mlp_channels],
            num_point=num_point,
            radii=[radius],
...
class cof_PointFPModule(BaseModule):
        ...
        for i in range(len(mlp_channels) - 1):
            self.mlps.add_module(
                ...
                ConvModule(
                    ...
                    conv_cfg=dict(type='Conv2d'),
                    norm_cfg=norm_cfg))
        # self.xyz_embeddings.add_module(
        #     f'layer{i}',
        #     )
        # )
        # self.cof_attention.add_module(
        #     f'layer{i}',
    ...
    def forward(self, target: torch.Tensor, source: torch.Tensor,
        ...
        # xyz_embedding = self.xyz_embeddings(target)
        new_features = self.cof_attention(target,
                                          new_features.permute(0, 2, 1).contiguous(),
                                          target.shape[1]).permute(0, 2, 1).contiguous()
        # mlp_features = []
        # for i in range(len(self.mlps)):
        #     tmp_feature = self.mlps[i](new_features)
        #     mlp_features.append(tmp_feature.squeeze(-1))
        #     new_features = tmp_feature
        #
        # cof_features = []
        # for i in range(len(self.mlps)):
        #     tmp_xyz_embedding = self.xyz_embeddings[i](target)
        #     tmp_cof_feature = self.cof_attention[i](tmp_xyz_embedding,
        #                                             mlp_features[i].permute(0, 2, 1).contiguous(),
        #                                             tmp_xyz_embedding.shape[1]).permute(0, 2, 1).contiguous()
        #     cof_features.append(tmp_cof_feature)
        # return mlp_features[-1], mlp_features, cof_features
        return new_features
