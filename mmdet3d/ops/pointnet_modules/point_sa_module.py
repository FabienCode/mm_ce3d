...
class CoF_BasePointSAModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.fps_mod_list = fps_mod
        self.fps_sample_range_list = fps_sample_range_list
        self.sa_layer_features = []
        self.layer_xyz = None
        self.xyz_embedding = None
        self.xyz_embedding1 = nn.Linear(3, mlp_channels[0][1])
        self.xyz_embedding2 = nn.Linear(3, mlp_channels[0][2])
        self.xyz_embedding3 = nn.Linear(3, mlp_channels[0][3])

        self.points_sampler = Points_Sampler(self.num_point, self.fps_mod_list,
                                             self.fps_sample_range_list)

        # self.register_buffer('layer_feature', None)
        ...
        # ========== cof modules
        self.CoF_attention = nn.ModuleList()
        for i in range(1, len(mlp_channels[0])):
            self.CoF_attention.append(BasicLayer(mlp_channels[0][i], 2, 32))

        self.cof_features = None
    ...
    def forward(
        ...
        new_xyz, indices = self._sample_points(points_xyz, features, indices,
                                               target_xyz)
        xyz_embedding.append(self.xyz_embedding1(new_xyz))
        xyz_embedding.append(self.xyz_embedding2(new_xyz))
        xyz_embedding.append(self.xyz_embedding3(new_xyz))
        self.xyz_embedding = xyz_embedding
        self.layer_xyz = new_xyz
        ...
        for i in range(len(self.groupers)):
            ...
            # - grouped_idx: (B, num_point, nsample)
            grouped_results = self.groupers[i](points_xyz, new_xyz, features)
            tmp_grouped_results = grouped_results

            # (B, mlp[-1], num_point, nsample)
            for j in range(len(self.mlps[i])):
                tmp_features = self.mlps[i][j](tmp_grouped_results)
                layer_feature_list.append(tmp_features)
                tmp_grouped_results = tmp_features
            ...
            if isinstance(self.mlps[i][0], PAConv):
                assert isinstance(new_features, tuple)
                new_features = new_features[0]
            for j in range(len(layer_feature_list)):
                layer_feature_list[j] = self._pool_features(layer_feature_list[j])

            # (B, mlp[-1], num_point)
            new_features = layer_feature_list[-1]
            new_features_list.append(new_features)

            self.sa_layer_features = layer_feature_list

            # ============= calculate cof_attention feature
            for j in range(len(self.mlps[i])):
                tmp_cof_feature = self.CoF_attention[j](xyz_embedding[j],
                                                        layer_feature_list[j].permute(0, 2, 1).contiguous(),
                                                        new_xyz.shape[1])
                tmp_xyz_embedding = self.xyz_embedding[j](new_xyz)
                cof_features.append(tmp_cof_feature)

            self.cof_features = cof_features

        return new_xyz, torch.cat(new_features_list, dim=1), indices, layer_feature_list

    # def get_layer_feature(self):
    #     'get the feature of each layer for CoF attention'
    #     return self.layer_feature


@SA_MODULES.register_module()
class CoF_PointSAModuleMSG(CoF_BasePointSAModule):
    """Point set abstraction module with multi-scale grouping (MSG) used in
    PointNets.
    ...
    def __init__(self,
                 ...
                 normalize_xyz=False,
                 bias='auto'):
        super(CoF_PointSAModuleMSG, self).__init__(
            num_point=num_point,
            radii=radii,
            sample_nums=sample_nums,
            mlp_channels=mlp_channels,
...
@SA_MODULES.register_module()
class CoF_PointSAModule(CoF_PointSAModuleMSG):
    """Point set abstraction module with single-scale grouping (SSG) used in
    PointNets.
    ...
    def __init__(self,
                 ...
                 fps_sample_range_list=[-1],
                 normalize_xyz=False):
        super(CoF_PointSAModule, self).__init__(
            mlp_channels=[mlp_channels],
            num_point=num_point,
            radii=[radius],
            sample_nums=[num_sample],
