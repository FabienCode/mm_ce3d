...
class cd_rr_BasePointSAModule(nn.Module):
    ...
    def __init__(self,
        ...
        super(cd_rr_BasePointSAModule, self).__init__()
    ...
    def _sample_points(self, points_xyz, features, indices, target_xyz):
        ...
        if indices is not None:
            ...
            if self.num_point is not None:
                if self.num_point[0] == 2048:
                    indices = self.points_sampler(points_xyz, features)
                    new_xyz = gather_points(xyz_flipped,
                                            indices).transpose(1, 2).contiguous()
                    dis_indices = torch.randint(points_xyz.shape[1], (self.num_point[0],), dtype=torch.int).unsqueeze(
                        0).repeat(
                        points_xyz.shape[0], 1).cuda()
                    # dis_indices = self.points_sampler(points_xyz, features)
                    dis_xyz = gather_points(xyz_flipped,
                                            dis_indices).transpose(1, 2).contiguous()

                else:
                    indices = self.points_sampler(points_xyz, features)
                    new_xyz = gather_points(xyz_flipped,
                                            indices).transpose(1, 2).contiguous()
                    dis_indices = (indices + torch.ones_like(indices, dtype=torch.int) * self.num_point[0]).cuda()
                    dis_xyz = gather_points(xyz_flipped,
                                            dis_indices).transpose(1, 2).contiguous()
            else:
                new_xyz = None

        return new_xyz, indices, dis_xyz, dis_indices
    ...
    def forward(
        ...
        new_xyz, indices, dis_xyz, dis_indices = self._sample_points(points_xyz, features, indices,
                                                                     target_xyz)
        ...
        for i in range(len(self.groupers)):
            ...
            for j in range(len(self.mlps[i])):
                ...
                ori_layer_features.append(ori_layer_feature)
            ...
            cof_layer1_feature = self.cof_attentions1(new_xyz,
                                                      ori_layer_features[0].permute(0, 2, 1).contiguous(),
                                                      ori_layer_features[0].shape[2])
            cof_layer_features.append(cof_layer1_feature)
            ...
            cof_layer2_feature = self.cof_attentions2(new_xyz,
                                                      ori_layer_features[1].permute(0, 2, 1).contiguous(),
                                                      ori_layer_features[1].shape[2])
            cof_layer_features.append(cof_layer2_feature)
            ...
            cof_layer3_feature = self.cof_attentions3(new_xyz,
                                                      ori_layer_features[2].permute(0, 2, 1).contiguous(),
                                                      ori_layer_features[2].shape[2])

            cof_layer_features.append(cof_layer3_feature)
            ...
            # =================== dis points feature norm
            dis_grouped_results = self.groupers[i](points_xyz, dis_xyz, features)

            # (B, mlp[-1], num_point, nsample)
            for j in range(len(self.mlps[i])):
                tmp_dis_feature = self.mlps[i][j](dis_grouped_results)
                tmp_layer_feature = self._pool_features(tmp_dis_feature).permute(0, 2, 1).contiguous()
                ln_layer_feature = F.layer_norm(tmp_layer_feature, (tmp_layer_feature.shape[-1],))
                layer_norm = torch.sqrt(torch.sum(ln_layer_feature ** 2, dim=(0, 1)))
                self.dis_points_norm.append(layer_norm)
                dis_grouped_results = tmp_dis_feature

            new_features_list.append(ori_layer_features[-1])
        return new_xyz, torch.cat(new_features_list, dim=1), indices, ori_layer_features, cof_layer_features


@SA_MODULES.register_module()
class cd_rr_PointSAModuleMSG(cd_rr_BasePointSAModule):
    ...
    def __init__(self,
                 ...
                 normalize_xyz=False,
                 bias='auto'):
        super(cd_rr_PointSAModuleMSG, self).__init__(
            num_point=num_point,
            radii=radii,
...
@SA_MODULES.register_module()
class cd_rr_PointSAModule(cd_rr_PointSAModuleMSG):
    ...
    def __init__(self,
                 ...
                 fps_sample_range_list=[-1],
                 normalize_xyz=False):
        super(cd_rr_PointSAModule, self).__init__(
            mlp_channels=[mlp_channels],
            num_point=num_point,
            radii=[radius],
...
class cd_rr_PointFPModule(BaseModule):
    ...
    def forward(self, target: torch.Tensor, source: torch.Tensor,
        ...
        for i in range(len(self.mlps)):
            ...
            ori_layer_features.append(ori_layer_feature.squeeze(-1))
            cof_layer_feature = self.cof_attentions[i](target,
                                                       ori_layer_feature.squeeze(-1).permute(0, 2, 1).contiguous(),
                                                       target.shape[1])
            cof_layer_features.append(cof_layer_feature)
            ln_cof_feature = F.layer_norm(cof_layer_feature, (cof_layer_feature.shape[-1],))
            cof_layer_norm = torch.sqrt(torch.sum(ln_cof_feature ** 2, dim=(0, 1)))
            self.cof_channels_norm.append(cof_layer_norm)
            new_features = ori_layer_feature

        return ori_layer_features[-1], ori_layer_features, cof_layer_features

class cd_rrn_PointFPModule(BaseModule):
    """Point feature propagation module used in PointNets.
    Propagate the features from one set to another.
    Args:
        mlp_channels (list[int]): List of mlp channels.
