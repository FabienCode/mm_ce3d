...
from mmdet3d.ops.pointnet_modules.point_fp_module import RR_PointFPModule
...
@BACKBONES.register_module()
class RR_PointNet2SASSG(BasePointNet):
    """PointNet2 with Single-scale grouping.
...
@BACKBONES.register_module()
class CoF_PointNet2SASSG(BasePointNet):
    """PointNet2 with Single-scale grouping.
    ...
    def forward(self, points):
        ...
        for i in range(self.num_sa):
            cur_xyz, cur_features, cur_indices, sa_layer_feature = self.SA_modules[i](
                sa_xyz[i], sa_features[i])
            sa_xyz.append(cur_xyz)
            sa_features.append(cur_features)
            sa_indices.append(
                torch.gather(sa_indices[-1], 1, cur_indices.long()))
            sa_layer_features.append(sa_layer_feature)
        ...
        ret = dict(
            ...
            sa_features=sa_features,
            sa_indices=sa_indices,
            sa_layer_features=sa_layer_features)
        return ret
