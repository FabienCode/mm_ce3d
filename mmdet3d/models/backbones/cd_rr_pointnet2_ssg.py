...
from mmdet3d.ops.pointnet_modules.cd_rr_pointnet import cd_rr_PointFPModule, cd_rrn_PointFPModule
from mmdet3d.core.utils.cof_attention import BasicLayer
from typing import List

@BACKBONES.register_module()
class cd_rr_PointNet2SASSG(BasePointNet):
    ...
    def __init__(self,
        ...
        for fp_index in range(len(fp_channels)):
            if fp_index != len(fp_channels) - 1:
                ...
                self.FP_modules.append(cd_rr_PointFPModule(mlp_channels=cur_fp_mlps))
                if fp_index != len(fp_channels) - 1:
                ...
                cur_fp_mlps = list(fp_channels[fp_index])
                cur_fp_mlps = [fp_source_channel + fp_target_channel] + cur_fp_mlps
                self.FP_modules.append(cd_rrn_PointFPModule(mlp_channels=cur_fp_mlps))
                if fp_index != len(fp_channels) - 1:
                    fp_source_channel = cur_fp_mlps[-1]
                    fp_target_channel = skip_channel_list.pop()
