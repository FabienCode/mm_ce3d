...
from mmdet3d.ops.pointnet_modules.cof_resrep_pointnet import cof_rr_PointFPModule, cof_rrc_PointFPModule


@BACKBONES.register_module()
class cof_rr_PointNet2SASSG(BasePointNet):
    ...
    def __init__(self,
        ...
        for fp_index in range(len(fp_channels)):
            if fp_index != len(fp_channels) - 1:
                ...
                self.FP_modules.append(cof_rrc_PointFPModule(mlp_channels=cur_fp_mlps))
                if fp_index != len(fp_channels) - 1:
                ...
                cur_fp_mlps = list(fp_channels[fp_index])
                cur_fp_mlps = [fp_source_channel + fp_target_channel] + cur_fp_mlps
                self.FP_modules.append(cof_rr_PointFPModule(mlp_channels=cur_fp_mlps))
                if fp_index != len(fp_channels) - 1:
                    fp_source_channel = cur_fp_mlps[-1]
                    fp_target_channel = skip_channel_list.pop()
    ...
    def forward(self, points):
        ...
        for i in range(self.num_sa):
            cur_xyz, cur_features, cur_indices, ori_feature, cof_feature = self.SA_modules[i](
                sa_xyz[i], sa_features[i])
            sa_xyz.append(cur_xyz)
            sa_features.append(cur_features)
            sa_indices.append(
                torch.gather(sa_indices[-1], 1, cur_indices.long()))
            for j in range(len(ori_feature)):
                ori_features.append(ori_feature[j])
                cof_features.append(cof_feature[j].permute(0, 2, 1).contiguous())
        ...
        for i in range(self.num_fp):
            fp_feature, fp_ori_feature, fp_cof_feature = self.FP_modules[i](sa_xyz[self.num_sa - i - 1],
            ...
            for j in range(len(fp_ori_feature)):
                ori_features.append(fp_ori_feature[j])
                cof_features.append(fp_cof_feature[j].permute(0, 2, 1).contiguous())
