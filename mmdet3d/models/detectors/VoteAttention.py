...
def extract_para(gt_bboxes_3d, gt_labels_3d):
    bbox_info = list()
    for i in range(len(gt_bboxes_3d)):
        bbox_info.append(gt_bboxes_3d[i].tensor)
    # # bbox_info = DC(gt_bboxes_3d.tensor)
    # bbox_info_cat = torch.stack(bbox_info)
    label_one_hot = list()
    for i in range(len(gt_labels_3d)):
        label_one_hot.append(to_one_hot(gt_labels_3d[i], 11))
    # bbox_label = torch.stack(gt_labels_3d)

    gt_para = list()
    for i in range(len(gt_labels_3d)):
        gt_para.append(torch.zeros((256, 18)))
        tmp = torch.cat((bbox_info[i], label_one_hot[i]), 1)
...
class VoteNetAttention(SingleStage3DDetector):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 pretrained=None):
        super(VoteNetAttention, self).__init__(
            backbone=backbone,
        ...
        #     self.pts_backbone = builder.build_backbone(backbone)
        # if label_backbone is not None:
        #     self.pts_backbone = builder.build_backbone(label_backbone)
    ...
    def forward_train(self,
        ...
        """
        # # Label information
        gt_para_cat = extract_para(gt_bboxes_3d, gt_labels_3d).permute(0, 2, 1).cuda()
        gt_para_cat = self.mlp1(gt_para_cat.unsqueeze(-1))
        gt_para_cat = self.mlp2(gt_para_cat)
        gt_para_cat = self.mlp3(gt_para_cat).permute(0, 2, 1, 3).squeeze(-1)

        points_cat = torch.stack(points)

        ori_feature = self.backbone(points_cat)
        ori_feature['fp_features'][0] = self.neck(ori_feature['fp_features'][0], gt_para_cat)
        ...
        losses['dict_fp2_loss'] = F.mse_loss(label_encodings, features)
